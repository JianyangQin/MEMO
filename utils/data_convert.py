...
class MinmaxScalar:
    ...
    # def transform(self, data):
    #     return (2. * data - (self.max + self.min)) / (self.max - self.min)
    #
    # def inverse_transform(self, data):
    #     return (data * (self.max - self.min) + (self.max + self.min)) / 2.
...
def generate_samples(days, savepath, data, graph, train_rate=0.6, val_rate=0.2, test_rate=0.2, val_test_mix=False,
    ...
    # std_scalar = StandardScalar(np.mean(train_x), np.std(train_x))
    #
    # train_x = std_scalar.transform(train_x)
    # val_x = std_scalar.transform(val_x)
    # test_x = std_scalar.transform(test_x)

    # np.savez(savepath, train_x=train_x, train_y=train_y, val_x=val_x, val_y=val_y, test_x=test_x, test_y=test_y,
    #          edge_index=edge_index, mean=std_scalar.mean, std=std_scalar.std)
    # data = {"train_x": train_x, "train_y": train_y, "val_x": val_x, "val_y": val_y, "test_x": test_x, "test_y": test_y,
    #         "edge_index": edge_index, "mean": np.asarray(std_scalar.mean), "std": np.asarray(std_scalar.std)}
